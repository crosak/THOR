...
__device__ void
bezier_interpolation(int id, int nlay, int iter, double *xi, double *yi, double x, double &y) {
...
__device__ void kernel_k_Ross_Freedman(double Tin, double Pin, double met, double &k_IR) {
    ...
    */
    ...
    k_lowP = c1 * atan(Tl10 - c2) - (c3 / (Pl10 + c4)) * exp(pow(Tl10 - c5, 2.0)) + c6 * met + c7;

    // Temperature split for coefficents = 800 K
    if (Tin <= 800.0) {
        k_hiP = c8_l + c9_l * Tl10 + c10_l * pow(Tl10, 2.0) + Pl10 * (c11_l + c12_l * Tl10)
                + c13_l * met * (0.5 + onedivpi * atan((Tl10 - 2.5) / 0.2));
    }
    else {
        k_hiP = c8_h + c9_h * Tl10 + c10_h * pow(Tl10, 2.0) + Pl10 * (c11_h + c12_h * Tl10)
...
__device__ void kernel_bilinear_log_interp(double  xval,
                                           double  yval,
                                           double  x1,
                                           double  x2,
                                           double  y1,
    ...
    aval = la11 * (lx2 - lxval) * (ly2 - lyval) * norm + la21 * (lxval - lx1) * (ly2 - lyval) * norm
...
__device__ void
kernel_k_Ross_Freedman_bilinear_interpolation_polynomial_fit(double  Tin,
                                                             double  Pin,
...
__device__ void linear_log_interp(int     id,
    ...
    Te__df_e[id * nlev + i + 1] =
        pow((double)(10.0), ((lTl1 * (lAlt2 - eAlt) + lTl2 * (eAlt - lAlt1)) * norm));
}
...
__device__ void
sw_grey_down(int id, int nlev, double Finc, double *solar_tau, double *sw_down__df_e, double *mu) {
    // dependencies
    //// expl -> math
...
__device__ void ts_short_char(int       id,
    ...
    if (mu_s[id] > 0.0) {
        ...
        for (int channel = 0; channel < 3; channel++) {
            ...
            sw_grey_down(id, nlev, Finc_B, tau_Ve__df_e, sw_down_b__df_e, mu_s);

            // Sum all bands
            for (int i = 0; i < nlev; i++) {
                sw_down__df_e[id * nlev + i] =
        ...
        */
        ...
        for (int level = 0; level < nv; level++) {
            ...
            */
            ...
            for (int channel = 0; channel < 3; channel++) {
                k_V_3_nv_d[id * nv * 3 + channel * nv + level] =
                    k_IR_2_nv_d[id * nv * 2 + 0 * nv + level] * gam_V_3_d[id * 3 + channel];
            }
            ...
            if (latf_lw) {
                ...
                k_IR_2_nv_d[id * nv * 2 + 1 * nv + level] =
                    k_IR_2_nv_d[id * nv * 2 + 0 * nv + level] * gam_2_d[id];
                k_IR_2_nv_d[id * nv * 2 + 0 * nv + level] =
                    k_IR_2_nv_d[id * nv * 2 + 0 * nv + level] * gam_1_d[id];

                k_IR_2_nv_d[id * nv * 2 + 1 * nv + level] =
                    k_IR_2_nv_d[id * nv * 2 + 1 * nv + level]
                    + k_IR_2_nv_d[id * nv * 2 + 1 * nv + level] * (kappa_lw_pole / kappa_lw)
                          * pow(sin(lonlat_d[id * 2 + 1]), 2);
                k_IR_2_nv_d[id * nv * 2 + 0 * nv + level] =
                    k_IR_2_nv_d[id * nv * 2 + 0 * nv + level]
                    + k_IR_2_nv_d[id * nv * 2 + 0 * nv + level] * (kappa_lw_pole / kappa_lw)
                          * pow(sin(lonlat_d[id * 2 + 1]), 2);
            }
            else {
                k_IR_2_nv_d[id * nv * 2 + 1 * nv + level] =
