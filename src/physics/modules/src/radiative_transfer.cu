...
bool radiative_transfer::phy_loop(ESP &                  esp,
    ...
    if (run) {
        ...
        if (rt_type == PICKETFENCE) {
                ...
                if (esp.insolation.get_host_cos_zenith_angles()[c] > 0.0) {
                    Teff[c] =
                        pow((pow(esp.Tint, 4.0)
                             + (1.0 - AB__h[c]) * esp.insolation.get_host_cos_zenith_angles()[c]
                                   * pow(Tirr, 4.0)),
                            0.25);
                }
                else {
            ...
            cudaStatus = cudaMemcpy(
                Beta_V_3_d, Beta_V__h, 3 * esp.point_num * sizeof(double), cudaMemcpyHostToDevice);
            if (cudaStatus != cudaSuccess) {
                fprintf(stderr, "Beta_V_3_d cudaMemcpyHostToDevice failed!");
                //goto Error;
            }
            cudaStatus = cudaMemcpy(
                Beta_2_d, Beta__h, 2 * esp.point_num * sizeof(double), cudaMemcpyHostToDevice);
            if (cudaStatus != cudaSuccess) {
                fprintf(stderr, "Beta_2_d cudaMemcpyHostToDevice failed!");
                //goto Error;
            }
            cudaStatus = cudaMemcpy(
                gam_1_d, gam_1__h, esp.point_num * sizeof(double), cudaMemcpyHostToDevice);
            if (cudaStatus != cudaSuccess) {
                fprintf(stderr, "gam_1_d cudaMemcpyHostToDevice failed!");
                //goto Error;
            }
            cudaStatus = cudaMemcpy(
                gam_2_d, gam_2__h, esp.point_num * sizeof(double), cudaMemcpyHostToDevice);
            if (cudaStatus != cudaSuccess) {
                fprintf(stderr, "gam_2_d cudaMemcpyHostToDevice failed!");
                //goto Error;
            }
            cudaStatus =
                cudaMemcpy(AB_d, AB__h, esp.point_num * sizeof(double), cudaMemcpyHostToDevice);
            if (cudaStatus != cudaSuccess) {
        ...
        if (nstep * time_step < (2 * M_PI / esp.insolation.get_mean_motion())) {
            ...
            // calculate annual average of insolation for the first orbit
...
bool radiative_transfer::store(const ESP &esp, storage &s) {
    ...
    if (rt_type == PICKETFENCE) {
        ...
        s.append_table(
            lw_net__h, esp.nvi * esp.point_num, "/lw_net__h", "W m^-2", "net long-wave flux (LW)");
        ...
        cudaMemcpy(sw_net__h,
                   ...
                   cudaMemcpyDeviceToHost);
        s.append_table(
            sw_net__h, esp.nvi * esp.point_num, "/sw_net__h", "W m^-2", "net short-wave flux (SW)");
        ...
        cudaMemcpy(flw_dn_h,
                   ...
                   cudaMemcpyDeviceToHost);
        s.append_table(
            flw_dn_h, esp.nvi * esp.point_num, "/flw_dn", "W m^-2", "downward flux (LW)");
        ...
        cudaMemcpy(fsw_dn_h,
                   ...
                   cudaMemcpyDeviceToHost);
        s.append_table(
            fsw_dn_h, esp.nvi * esp.point_num, "/fsw_dn", "W m^-2", "downward flux (SW)");
        ...
        cudaMemcpy(
            qheat_h, qheat_d, esp.nv * esp.point_num * sizeof(double), cudaMemcpyDeviceToHost);
    ...
    if (rt_type == FREEDMAN) {
        ...
        cudaMemcpy(
            insol_ann_h, insol_ann_d, esp.point_num * sizeof(double), cudaMemcpyDeviceToHost);
        ...
        cudaMemcpy(
            tau_h, tau_d, esp.nv * esp.point_num * 2 * sizeof(double), cudaMemcpyDeviceToHost);
        s.append_table(tau_h,
...
bool radiative_transfer::store_init(storage &s) {
    ...
    //      rt type  option
    s.append_value(int(rt_type), "/rt_type", "-", "Type of radiative transfer physics");
